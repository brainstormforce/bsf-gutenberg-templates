...
function CategoryItem( { category, isActive, onClick, disableAi } ) {
	...
	const currentScreenValue =
		currentScreen === 'all-block-pages-grid' ? 'pages' : 'patterns';
	...
	const isSyncing = () => {
		...
		if (
			( category.id === 'favorite' || category.id === '' ) &&
			dynamicContentSyncStatus[ currentScreenValue ]
		) {
			return true;
		}

		if (
			syncFlags[ category.id ] === false &&
			dynamicContentSyncStatus[ currentScreenValue ]
		) {
			return true;
		}
	...
	const handleRegenerateAI = async ( event ) => {
		...
		let succeeded = false;
		if ( ! category?.id ) {
			// Regenerate all categories. Disabled for now.
			succeeded = await generateContentForAllCategories(
				mergedCategories,
				setDynamicContent,
				dynamicContentFlagSet,
				( catValue ) =>
					setCurrentCategory( currentScreenValue, catValue ),
				setCreditsDetails,
	...
	const showPagesSparkleIcon =
			currentScreenValue === 'pages' &&
			!! dynamicContent &&
			! dynamicContent.hasOwnProperty( category.id ),
		isReGenerating =
			reSyncStatus && regeneratingContentCategory === category.id;
...
const Categories = ( {
	...
	if ( loadingBlocksAndSites ) {
		return Array.from( { length: 5 } ).map( ( _, indx ) => (
			<div
				key={ indx }
				data-placeholder
				className="relative w-full h-10 rounded bg-gray-200"
			/>
		) );
	}
