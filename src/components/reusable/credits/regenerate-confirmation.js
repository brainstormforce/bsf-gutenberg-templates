			...
			trigger={ ( { open: active } ) => (
				<button
					className={ classNames(
						'[&:has(.animate-none)]:!visible group-hover:visible flex items-center justify-center group border-0 focus:outline-none bg-transparent rounded cursor-pointer',
						! active && 'invisible'
					) }
					disabled={ disabled }
				>
				...
				{ ( { close } ) => (
					...
					>
						...
						<div className="flex flex-col gap-2">
							<Button
								className={ classNames(
									'w-full bg-background-tertiary text-accent-spectra !text-sm',
									isDisabled &&
										'bg-button-disabled text-secondary-text'
								) }
								variant="blank"
								...
								disabled={ isDisabled }
							>
								{ boxText } ({ formatNumber( consumptionRate ) }{ ' ' }
								Credits)
							</Button>
							<p
								className={ classNames(
									'w-full m-0 p-0 text-sm font-normal text-secondary-text text-center',
							...
							<div
								onClick={ () =>
									handleClickGetMoreCredits( close )
								}
