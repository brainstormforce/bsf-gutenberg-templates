...
const HeaderAIDropdown = ( {
	...
	const {
		aiStepData: { token, businessDetails, selectedImages },
	} = useSelect( ( select ) => {
	...
	const resetDetails = () => {
		...
		formData.append(
			'action',
			'ast-block-templates-reset-business-details'
		);
		formData.append(
			'security',
	...
	const options = [
		...
		},
		{
			...
			isHide: ! token || ! selectedImages?.length || ! isPersonalized,
			isDisabled:
				disableAi || ! adaptiveMode || ! token || ! businessDetails,
		},
		{
			label: () => {
			...
			label: () => {
				return (
					<div className="flex justify-between">
						<span>
							{ __( 'Disable AI', 'ast-block-templates' ) }
						</span>
						<button
							type="button"
		...
		},
		{
			...
			description: __(
				'Disabling Adaptive Mode shows templates without personalized text, images, and theme styling.',
				'ast-block-templates'
			),
			icon: (
...
export default compose(
	withSelect( ( select ) => {
		...
		return {
			...
			isPersonalized: getIsPersonalized(),
		};
	} ),
