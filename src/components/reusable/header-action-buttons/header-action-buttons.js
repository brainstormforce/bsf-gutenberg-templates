...
const HeaderActionButtons = ( {
	...
	const { setTogglePopup, setAllSites, setBackgroundSyncStatus } = useDispatch( STORE_KEY );
	const { stepData, syncProgressStatus } = useSelect( ( select ) => {
		const { getOnboardingAI, getBackgroundSyncStatus } = select( STORE_KEY );
		return {
			...getOnboardingAI(),
			syncProgressStatus: getBackgroundSyncStatus(),
		};
	}, [] );
		...
		await post( {
			...
			if ( response.status ) {
				...
				if ( i === total ) {
					...
					//setting the sync progress to no so as to remove the Syncing badge.
					setBackgroundSyncStatus( 'no' );
					// Updated UI.
					selfSetState( {
	...
	return (
		<div className="justify-self-end flex items-center gap-0.5 sm:gap-1 md:gap-3 w-max">
