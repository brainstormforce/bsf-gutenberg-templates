...
const SingleSitePage = ( {
	...
	return (
		<div
			className={ classNames( 'w-full content-start relative' ) }
			onClick={ () => {
				if ( importInProgress ) {
					return;
				}
		...
		>
			<div
				className={ classNames(
					...
					importInProgress && 'cursor-not-allowed pointer-events-none'
				) }
			>
				<div
					className={ classNames(
				...
				<div
					className={ classNames(
						...
						pagePreview.ID === item.ID
							? 'border-accent-spectra shadow-large'
							: ''
					) }
				>
					<div
...
export default compose(
	withSelect( ( select ) => {
		const { getPagePreview, getImportInProgress } = select( 'ast-block-templates' );
		return {
