...
/**
 * Find the best insertion target for the blocks among the selected block and its ancestors.
 *
 * @param {Array}    blocks                The blocks to be inserted
 * @param {string}   selectedBlockClientId The client ID of the selected block
 * @param {Function} canInsertBlockType    Function to check if a block type can be inserted
 * @param {Function} getBlockRootClientId  Function to get the root client ID of a block
 * @return {string|boolean} The target client ID or false if no suitable target found
 */
export const findInsertTarget = ( blocks, selectedBlockClientId, canInsertBlockType, getBlockRootClientId ) => {
	if ( ! blocks?.length ) {
		return false;
	}

	let current = selectedBlockClientId;

	while ( current ) {
		const canInsertHere = blocks.every( ( block ) =>
			canInsertBlockType( block.name, current )
		);
	...
	// Explicit root check
	const canInsertAtRoot = blocks.every( ( block ) =>
		canInsertBlockType( block.name, null )
	);

	return canInsertAtRoot ? '' : false;
};
