...
import { logError } from '../components/reusable/error-toast/handle-error';
...
export const getAuthURL = () => {
	const { spec_ai_auth_url: specAiUrl } = ast_block_template_vars;
	const siteURL =
		window.location.href +
		'&ast_action=auth&nonce=' +
		ast_block_template_vars.zipwp_ai_auth_nonce;
	return `${ specAiUrl }&redirect_url=${ encodeURIComponent( siteURL ) }`;
};

export const getPopupAuthURL = () => {
	const { spec_ai_auth_url: specAiUrl } = ast_block_template_vars;
	const siteURL =
